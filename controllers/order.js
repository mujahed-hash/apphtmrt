...
const getAllOrders = asyncHandler(async (req, res) => {
  ...
  const limit = parseInt(req.query.limit) || 20;
    const orders = await Order.find().sort({date:-1}).populate('customIdentifer').populate('orderItems.product', 'prodName prodPrice images').populate('user')
    .skip(start)
    .limit(limit);
...
const getOrders = asyncHandler(async (req, res) => {
  const orders = await Order.find({ user: req.userId }).sort({date:-1}).populate('orderItems.product', 'prodName prodPrice images');
...
const getSupplierOrders = async (supplierId) => {
    try {
      ...
      const orders = await Order.find({ 'orderItems.product': { $in: productIds } }).sort({date:-1})
        .populate({
          path: 'orderItems.product',
          select: 'prodName prodDesc prodPrice',
        })
        .populate({
          path: 'user',
          select: 'name email',
